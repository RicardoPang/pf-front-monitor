...
class SaveLogBase extends Base {
  ...
  autoCloseOldStream(isCloseAll = false) {
    ...
    for (
      ...
      let survivalAtLogUri = Nginx.getAbsoluteLogUriByType(
        nowAt,
        Nginx.LOG_TYPE_JSON
      );
      let survivalAtRawLogUri = Nginx.getAbsoluteLogUriByType(
        nowAt,
        Nginx.LOG_TYPE_RAW
      );
      if (isCloseAll === false) {
  ...
  async parseLog(data, projectMap) {
    ...
    if (record.ip === "127.0.0.1") record.ip = "59.109.146.215";
